param(
    ...
    [string]$Root = "/sdcard",
    [string]$TaskName = "Duplicate & Transfer Manager ADB Cache",
    [string]$Day = "Sunday",
    [string]$At = "03:00"
)
...
Register-ScheduledTask -TaskName $TaskName -Action $action -Trigger $trigger -Settings $settings -Description "Refresh the Duplicate & Transfer Manager ADB cache when the phone is connected." -Force
